...
import featureSeqDiag.sdcombin.runner.SdcRunner;
...
public class FeatureSDComposer extends ComposerExtensionClass {
	...
	public void performFullBuild(IFile config) {
		...
		if (p != null) {
			...
			//
			if ((dir.mkdirs()) == true) {
				// // On prend le fichier SysCombin.sdc créer par l'utilsateur
				...
				assert (fileSysCombin.exists()) : "File SysCombin.sdc not found! ";

				Feature fRoot = configuration.getFeatureModel().getRoot();
				SequenceProg sysComb = SdcRunner
						.ParseSDCProgram(fileSysCombin.getAbsolutePath());
				...
				// Writing the new newSysCombFile.sdc after cleaning
				try {
				...
				String sdeOut = SDGenerator.unifyingSDfile(sysComb,					
						spconfig[0] + "_config_" + "FinalSD.sd", separator);
				// Writing the new newSysCombFile.sdc after cleaning
				try {
